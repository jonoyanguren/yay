...
export default function RetreatForm({ retreat, isEdit = false }: RetreatFormProps) {
  ...
  const [activities, setActivities] = useState<string[]>(retreat?.activities || []);
  const [program, setProgram] = useState<string[]>(retreat?.program || []);
  const [arrivalOptions, setArrivalOptions] = useState<ArrivalOption[]>(retreat?.arrivalOptions || []);
  const [dayByDay, setDayByDay] = useState<DayByDay[]>(retreat?.dayByDay || []);
  const [includes, setIncludes] = useState<string[]>(retreat?.includes || []);
  const [notIncludes, setNotIncludes] = useState<string[]>(retreat?.notIncludes || []);
  const [extraIdeas, setExtraIdeas] = useState<string[]>(retreat?.extraIdeas || []);
  ...
  const [newArrivalOption, setNewArrivalOption] = useState({ title: "", detail: "" });
  ...
  const addRoomType = () => {
    if (newRoomType.name.trim() && newRoomType.priceCents && newRoomType.maxQuantity) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Información Básica</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              disabled={isEdit}
            />
            <p className="text-xs text-slate-500 mt-1.5">Identificador único para la URL (no se puede cambiar)</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Ubicación</label>
            <input
              type="text"
              name="location"
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Fechas</label>
            <input
              type="text"
              name="date"
              value={formData.date}
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">URL de Imagen</label>
            <input
              type="text"
              name="image"
        ...
        <div className="mt-5 pt-4 border-t border-slate-100">
          <label className="flex items-center cursor-pointer">
            ...
            />
            <span className="ml-2 text-sm font-medium text-slate-700">Publicado</span>
            <span className="ml-2 text-xs text-slate-500">(visible en la web)</span>
          </label>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Descripción Corta</label>
            <textarea
              name="description"
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Descripción Completa</label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Intro Llegadas</label>
            <textarea
              name="arrivalIntro"
      ...
      {/* Activities */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Actividades Destacadas</h2>
        ...
        {activities.length > 0 && (
          <ul className="space-y-2">
            {activities.map((activity, index) => (
              <li key={index} className="flex items-center justify-between bg-slate-50 px-4 py-3 rounded-lg border border-slate-100">
      ...
      {/* Arrival Options */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Opciones de Llegada</h2>
        <div className="grid grid-cols-1 gap-3 mb-4">
          <input
            type="text"
            value={newArrivalOption.title}
            onChange={(e) => setNewArrivalOption({...newArrivalOption, title: e.target.value})}
          ...
          <input
            ...
            onChange={(e) => setNewArrivalOption({...newArrivalOption, detail: e.target.value})}
      ...
      {/* Day by Day */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Itinerario Día a Día</h2>
        <div className="grid grid-cols-1 gap-3 mb-4">
          <input
            type="text"
            value={newDayByDay.day}
            onChange={(e) => setNewDayByDay({...newDayByDay, day: e.target.value})}
            placeholder="Día (Ej: Día 1)"
            className="px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-shadow"
          />
          <textarea
            value={newDayByDay.items}
            onChange={(e) => setNewDayByDay({...newDayByDay, items: e.target.value})}
      ...
      {/* Extra Ideas */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Otras Ideas (tarde/noche)</h2>
        <div className="flex gap-2 mb-4">
          <input
            type="text"
            value={newExtraIdea}
            onChange={(e) => setNewExtraIdea(e.target.value)}
            onKeyPress={(e) => e.key === "Enter" && (e.preventDefault(), addExtraIdea())}
        ...
        {extraIdeas.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {extraIdeas.map((idea, index) => (
              <div key={index} className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-full border border-slate-100">
      ...
      {/* Extra Activities */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        <h2 className="text-xl font-bold mb-5 text-slate-800">Actividades Extra</h2>
        <p className="text-sm text-slate-600 mb-4">Actividades opcionales que se pueden añadir a la reserva</p>
        <div className="grid grid-cols-1 gap-3 mb-4">
          <input
            type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              ...
              onChange={(e) => setNewExtraActivity({...newExtraActivity, priceCents: e.target.value})}
            ...
            <input
              ...
              onChange={(e) => setNewExtraActivity({...newExtraActivity, maxQuantity: e.target.value})}
          ...
          <label className="flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => setNewExtraActivity({...newExtraActivity, allowMultiple: e.target.checked})}
              className="w-4 h-4 text-emerald-600 border-slate-300 rounded focus:ring-emerald-500"
            />
            <span className="ml-2 text-sm font-medium text-slate-700">Permitir múltiples unidades</span>
          </label>
        </div>
        <button
          type="button"
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 mt-2 text-sm text-slate-600">
                      ...
                      <span className={activity.allowMultiple ? "text-emerald-600" : "text-slate-400"}>
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-lg shadow sticky bottom-4">
        ...
        >
          {isLoading ? "Guardando..." : isEdit ? "Actualizar Retiro" : "Crear Retiro"}
        </button>
      </div>
    </form>
  );
